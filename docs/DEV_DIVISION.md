...
| 2.7 列表页城市/日期筛选头与弹层 | **B** | 写一个文档（置于docs）说明在较短时间内可达到的实现情况，毕竟所有城市都做会很难。先弄几个典型的城市。补全未在搜索框右侧加上的搜索icon以及搜索逻辑，如果需要调接口，做好注释，同时为后端预留位置 |
| 2.8 列表页详细筛选区域 | **B** |即搜索页下方的四个筛选项“位置/距离”“价格/等级”“居数/床数”“筛选/排序”都做下拉筛选框如例图所示，无需像例图一样有过多功能，简洁设计这部分的筛选即可 |
| 2.9 列表页分页与上滑加载更多 | **B** | 写文档到docs，其中精炼描述这部分逻辑的处理方式，写好可以在答辩时候说明的设计理念，以及页面加载等优化亮点 |
| 2.10 详情页大图左右滑动 | **B** | |
| 2.11 详情页名称/设施/地址 | **B** | |
| 2.12 详情页房型价格列表（按价格从低到高） | **B** |至此请通读整个项目的相关页面代码逻辑，确保酒店列表和详情页基本完善，实现了目前未实现的列表页城市选和时间选择以及关键字搜索 |
